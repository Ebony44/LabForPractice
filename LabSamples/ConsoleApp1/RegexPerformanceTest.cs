...
namespace ConsoleApp1
{
    class RegexPerformanceTest
    {
        ...
        static RegexPerformanceTest()
        {
            uniRules = MakeUniRules;
            zgRules = MakeZGRules;
        }

        private static List<Rule> MakeUniRules
        {
            get
            {
        ...
        private static List<Rule> MakeZGRules
        {
            get
            {
                List<Rule> rules = new List<Rule>();
        ...
        public static string Uni2ZGWithInstantiation(string Uni)
        {
            if (Uni == null)
                return null;

            var result = Convert(uniRules, Uni);
            //return StringFilterUtil.RemoveLineFilter(result);

            return Uni;

        }

        public static string Uni2ZGWithoutInstantiation(string Uni)
        {
            if (Uni == null)
                return null;
            ...
            // var Rules = uniRules;
            // Regex rgx2 = new Regex(Rules[0].From);
            // input = Regex.Replace(input, Rules[0].From, Rules[0].To);
            foreach (Rule r in uniRules)
            {
                // Regex rgx = new Regex(r.From);
                // input = rgx.Replace(input, r.To);
                Uni = Regex.Replace(Uni, r.From, r.To);
            }
            // Uni = input;
            return Uni;
